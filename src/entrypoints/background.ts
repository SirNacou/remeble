...
import { AnkiResponse, postAnkiConnectRequest } from "@/api/anki_connect";
...
export default defineBackground({
  main() {
    ...
    browser.runtime.onMessage.addListener((request, sender, sendResponse) => {
      // Check if the message is specifically for AnkiConnect
      if (request.action === "postAnkiConnect") {
        return postAnkiConnect(request, sendResponse);
      }
...
function postAnkiConnect(request: any, sendResponse: (response?: any) => void) {
  setTimeout(async () => {
    const payload = request.payload;
    console.log("Background: Received AnkiConnect request:", payload);
    let res: AnkiResponse<any> | null = null;
    try {
      res = await postAnkiConnectRequest(payload);
    } catch (error) {
